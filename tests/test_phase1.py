...
class TestPhase1BasicFunctionality(unittest.TestCase):
    ...
    def test_api_key_validation_failure(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            from config.llm_config import validate_api_key
            self.assertFalse(validate_api_key(), "API key validation should fail without key")
    ...
    def test_validate_game_data_structure(self):
        ...
        self.assertTrue(result.get("is_complete", False), "Complete data should validate as complete")
        self.assertEqual(result.get("completeness_score", 0), 100.0, "Complete data should have 100% score")
    ...
    def test_autogen_agents_creation(self):
        ...
        try:
            ...
            with patch('autogen.AssistantAgent') as mock_assistant, \
                ...
                self.assertTrue(mock_assistant.called, "AssistantAgent should be called")
    ...
    def test_data_collection_workflow(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                self.assertIn("completeness_score", validation, "Should have completeness score")
            else:
                # Jeśli błąd, sprawdź komunikat
                self.assertIn("message", result, "Error result should have message")
                
        except Exception as e:
            self.skipTest(f"Skipping integration test due to network/scraping issue: {e}")
