...
def test_advanced_analysis(game_name: str) -> None:
    ...
    try:
        ...
        if not game_data.get("success", False):
            print(f"❌ Failed to get game data: {game_data.get('message', 'Unknown error')}")
            return
        ...
        if not advanced_analysis.get("success", False):
            print(f"❌ Advanced analysis failed: {advanced_analysis.get('error', 'Unknown error')}")
            return
        ...
        if market_analysis:
            print(f"\n📈 MARKET ANALYSIS:")
            print(f"  💸 Price Category: {market_analysis.get('price_category', 'Unknown')}")
            print(f"  ⭐ Quality Category: {market_analysis.get('quality_category', 'Unknown')}")
            print(f"  🎯 Market Position: {market_analysis.get('market_position', 'Unknown')}")
        ...
        if genre_analysis:
            print(f"\n🎮 GENRE ANALYSIS:")
            print(f"  🏷️ Primary Genre: {genre_analysis.get('primary_genre', 'Unknown')}")
            print(f"  ⏱️ Expected Hours: {genre_analysis.get('expected_hours', 0)}")
            print(f"  💰 Cost per Hour: {genre_analysis.get('cost_per_hour', 0):.2f}")
            print(f"  📊 Genre Value Score: {genre_analysis.get('final_value_score', 0)}")
            print(f"  🔥 Developer Multiplier: {genre_analysis.get('developer_multiplier', 1.0)}")
        ...
        if value_breakdown:
            print(f"\n💎 VALUE BREAKDOWN:")
            print(f"  🎮 Genre Contribution: {value_breakdown.get('genre_contribution', 0)}")
            print(f"  📈 Market Contribution: {value_breakdown.get('market_contribution', 0)}")
            print(f"  📅 Age Contribution: {value_breakdown.get('age_contribution', 0)}")
...
def compare_basic_vs_advanced(game_name: str) -> None:
    ...
    try:
        ...
        if basic_result.get("success"):
            basic_rec = basic_result.get("value_metrics", {}).get("recommendation", "Unknown")
            basic_score = basic_result.get("value_metrics", {}).get("value_score", 0)
            basic_timing = basic_result.get("value_metrics", {}).get("buy_timing", "Unknown")
        ...
        if advanced_result.get("success"):
            ...
            market_pos = comp_analysis.get("market_analysis", {}).get("market_position", "Unknown")
            value_tier = comp_analysis.get("market_analysis", {}).get("value_tier", "Unknown")
